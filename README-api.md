...
---
* ##### _同步创建布局中标记的所有子模块：_ `createTemplateModules(configMap)`

	* 参数说明：
		```
		{Object}   configMap <可选> [子模块与子模块配置的对象]
		```

	* 返回值：无

	* 特殊说明：每个模块的 target 参数都是用标记的 DOM 节点，用来简化多个子模块的创建，只需在 DOM 节点上标记子模块名称即可
	...
	* 用法示例：
		...
		var Page = sugar.Container.extend({
			...
			viewReady: function() {
				// 子模块的配置，每个配置不需要指定 target 参数，因为标记的 DOM 节点就是子模块创建的目标容器
				var subsConfig = {
					'header': {
						'module': header,
						'config': {/* header模块配置参数 */}
				...
				this.createTemplateModules(subsConfig);
			}
		});
		```
