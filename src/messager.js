...
define([
	 ...
	 * @return {Object}
	 */
	p._create = function(type, sender, name, param) {
		return {
			// 消息类型
			'type'   : type,
			// 消息发起模块实例
	 ...
	 * @return {Mix}
	 */
	p._trigger = function(receiver, msg, returns) {
		// 接收者对该消息的接收方法
	 ...
	 */
	p._notifySender = function(msg, callback, context) {
		// callback未定义时触发默认事件
		if (!callback) {
	 ...
	 * @param  {Object}    context   [执行环境]
	 */
	p.fire = function(sender, name, param, callback, context) {
		var type = 'fire';
	 ...
	 */
	p.broadcast = function(sender, name, param, callback, context) {
		var type = 'broadcast';
	 ...
	 * @param  {Object}    context   [执行环境]
	 */
	p.notify = function(sender, receiver, name, param, callback, context) {
		var type = 'notify';
	 ...
	 * @param  {Mix}     param  [<可选>附加消息参数]
	 */
	p.globalCast = function(name, param) {
		var type = 'globalCast';
