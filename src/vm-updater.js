...
define([
	 ...
	 * @param   {String}      text
	 */
	p.updateNodeTextContent = function(node, text) {
	 ...
	 */
	p.updateNodeHtmlContent = function(node, html, isPlain) {
	 ...
	 */
	p.updateNodeDisplay = function(node, show) {
	 ...
	 * @param  {DOMElement}  node
	 */
	p.setNodeVisibleDisplay = function(node) {
	 ...
	 */
	p.updateNodeRenderContent = function(node, isRender) {
		var siblingNode = node.nextSibling;
	 ...
	 * 缓存节点渲染内容并清空
	 */
	p.setNodeRenderContent = function(node) {
	 ...
	 * 切换节点内容渲染
	 */
	p.toggleNodeRenderContent = function(node, isRender) {
		var fragment;
		// 渲染
		if (isRender) {
	 ...
	 * @param   {String}      value
	 */
	p.updateNodeAttribute = function(node, attribute, value) {
		if (value === null) {
	 ...
	 */
	p.updateNodeClassName = function(node, newValue, oldValue, classname) {
	 ...
	 */
	p.updateNodeStyle = function(node, propperty, value) {
	 ...
	 */
	p.updateNodeEvent = function(node, evt, func, oldFunc, params, field, index) {
	 ...
	 * @param   {String} value
	 */
	p.updateNodeFormTextValue = function(text, value) {
		if (text.value !== value) {
			text.value = value;
		}
	},
	 ...
	 * @param   {String} value
	 */
	p.updateNodeFormRadioChecked = function(radio, value) {
	 ...
	 */
	p.updateNodeFormCheckboxChecked = function(checkbox, values) {
	 ...
	 * @param   {Boolean}        multi
	 */
	p.updateNodeFormSelectChecked = function(select, selected, multi) {
