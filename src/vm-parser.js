...
define([
	 ...
	 * v-el
	 */
	p.parseVEl = function(node, value, name, fors) {
	 ...
	 * v-text, {{text}} DOM文本
	 */
	p.parseVText = function(node, value, name, fors) {
		var access, replace;
	 ...
	 * v-html DOM布局
	 */
	p.parseVHtml = function(node, value, name, fors) {
	 ...
	 * v-show 控制节点的显示隐藏
	 */
	p.parseVShow = function(node, value, name, fors) {
		var access;
		var updater = this.updater;
		var watcher = this.watcher;
		var inFor = fors && this.isForValue(value, fors);
	 ...
	 * v-if 控制节点内容的渲染
	 */
	p.parseVIf = function(node, value, name, fors) {
		var access;
		var updater = this.updater;
		var watcher = this.watcher;
		var inFor = fors && this.isForValue(value, fors);
	 ...
	 */
	p.parseVBind = function(node, value, attr, fors) {
	 ...
	 * @param   {Array}        fors
	 */
	p.bindClassName = function(node, field, classname, fors) {
		var updater = this.updater;
	 ...
	 */
	p.bindClassNameVfor = function(node, field, classname, fors) {
		var updater = this.updater;
		var watcher = this.watcher;
	 ...
	 */
	p.watchClassNameObject = function(node, access, classObject) {
	 ...
	 */
	p.bindClassNameObject = function(node, newObject, oldObject) {
		var updater = this.updater;
	 ...
	 * @param   {Array}       fors
	 */
	p.bindInlineStyle = function(node, field, propperty, fors) {
		var updater = this.updater;
	 ...
	 */
	p.bindInlineStyleVfor = function(node, field, fors) {
		var updater = this.updater;
		var watcher = this.watcher;
	 ...
	 * @param   {Object}  classObject
	 */
	p.watchInlineStyleObject = function(node, access, styleObject) {
	 ...
	 */
	p.bindInlineStyleObject = function(node, newObject, oldObject) {
		var updater = this.updater;
	 ...
	 * @param   {Array}       fors
	 */
	p.bindNormalAttribute = function(node, field, attr, fors) {
	 ...
	 * v-on 动态绑定一个或多个事件
	 */
	p.parseVOn = function(node, value, attr, fors) {
	 ...
	 */
	p.parseVOnEvent = function(node, field, args, evt, fors) {
		var access;
		var watcher = this.watcher;
		var updater = this.updater;
		var inFor = fors && this.isForValue(field, fors);
	 ...
	 * v-model 表单控件双向绑定
	 */
	p.parseVModel = function(node, field) {
	 ...
	 */
	p.parseVModelText = function(node, field, name, fors) {
		var access;
		var updater = this.updater;
		var watcher = this.watcher;
		var inFor = fors && this.isForValue(field, fors);
	 ...
	 * @param   {Array}    fors
	 */
	p.bindVModelTextEvent = function(node, field, inFor, fors) {
	 ...
	 * v-model for radio
	 */
	p.parseVModelRadio = function(node, field, name, fors) {
		var access;
		var updater = this.updater;
		var watcher = this.watcher;
		var inFor = fors && this.isForValue(field, fors);
	 ...
	 * @param   {String}  field
	 */
	p.bindVModelRadioEvent = function(node, field, inFor, fors) {
		var self = this;
		dom.addEvent(node, 'change', function() {
			self.setVModelValue(field, this.value, inFor, fors);
		});
	 ...
	 */
	p.parseVModelCheckbox = function(node, field, name, fors) {
		var watcher = this.watcher;
		var updater = this.updater;
	 ...
	 * @param   {Array}    infos
	 */
	p.bindVModelCheckboxEvent = function(node, field, inFor, infos) {
	 ...
	 * v-model for select
	 */
	p.parseVModelSelect = function(node, field, name, fors) {
		var updater = this.updater;
		var watcher = this.watcher;
		var inFor = fors && this.isForValue(field, fors);
	 ...
	 * @param   {Array}    fors
	 */
	p.bindVModelSelectEvent = function(node, field, multi, inFor, fors) {
		var self = this;
		dom.addEvent(node, 'change', function() {
	 ...
	 * @return  {Array}
	 */
	p.getSelectValue = function(select) {
	 ...
	 * @param   {Select}  select
	 */
	p.froceUpdateOption = function(select, fors) {
	 ...
	 * @param  {Array}    fors
	 */
	p.setVModelValue = function(field, value, inFor, fors) {
		var key;
		if (inFor) {
			key = this.getVforKey(field);
			fors[0][key] = value;
	 ...
	 * v-for 基于源数据重复的动态列表
	 */
	p.parseVFor = function(node, value, attr, fors) {
	 ...
	 */
	p.buildVforTemplate = function(node, array, field, scope, alias, level) {
		var vm = this.vm;
	 ...
	 */
	p.differVfors = function(parent, node, newArray, method, infos) {
	 ...
	 * @return  {FirstChild}
	 */
	p.getVforFirstChild = function(parent, alias) {
	 ...
	 * @return  {LastChild}
	 */
	p.getVforLastChild = function(parent, alias) {
	 ...
	 */
	p.pushVforArray = function(parent, node, newArray, method, infos) {
		var lastChild;
	 ...
	 */
	p.unshiftVforArray = function(parent, node, newArray, method, infos) {
		var firstChild;
		var fragment = util.createFragment();
		var cloneNode = node.cloneNode(true);
		var field = infos[0], scope = infos[1], alias = infos[2], level = infos[3];
	 ...
	 * 重新编译循环体数组
	 */
	p.recompileVforArray = function(parent, node, newArray, method, infos) {
	 ...
	 * @return  {Boolean}
	 */
	p.isForValue = function(field, fors) {
	 ...
	 */
	p.getVforValue = function(value, fors) {
	 ...
	 */
	p.getVforAccess = function(value, fors) {
	 ...
	 * @return  {String}
	 */
	p.replaceVforIndex = function(expression, index) {
