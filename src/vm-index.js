...
define([
	 ...
	 */
	p.complieElement = function(element, root, fors) {
		var node, dirCount;
	 ...
	 */
	p.complieDirectives = function(info) {
	 ...
	 * @param   {Array}           fors
	 */
	p.compile = function(node, directive, fors) {
	 ...
	 * @param   {Array}        fors
	 */
	p.compileTextNode = function(node, fors) {
		var text = node.textContent;
	 ...
	 * @param   {DOMElement}  node
	 */
	p.blockCompileNode = function(node) {
		util.each(this.$unCompileNodes, function(info) {
			if (node === info[0]) {
				return null;
			}
	 ...
	 * @return  {Boolean}
	 */
	p.isElementNode = function(element) {
	 ...
	 * @return  {Boolean}
	 */
	p.isDirective = function(directive) {
	 ...
	 * @return  {Boolean}
	 */
	p.isLateCompileNode = function(node) {
	 ...
	 * 根节点编译完成，更新视图
	 */
	p.rootComplieCompleted = function() {
	 ...
	 * @param  {Mix}     value
	 */
	p.setValue = function(field, value) {
